...
ProcessEvent(SDL_Event *event) {
	...
	case SDL_KEYUP:
		...
		if(savefpKeyts != NULL) {
			...
			ga_save_printf(savefpKeyts, "KEY-UP: %u.%06u scan 0x%04x sym 0x%04x mod 0x%04x\n",
	...
	case SDL_KEYDOWN:
		...
		if(savefpKeyts != NULL) {
			gettimeofday(&tv, NULL);
			ga_save_printf(savefpKeyts, "KEY-DN: %u.%06u scan 0x%04x sym 0x%04x mod 0x%04x\n",
				tv.tv_sec, tv.tv_usec,
				event->key.keysym.scancode,
				event->key.keysym.sym,
				event->key.keysym.mod);
...
int InitiatingConnectionToServer(LPCSTR IPAddress) {
	WSADATA wsaData;
	SOCKET connectSocket = INVALID_SOCKET;
	struct addrinfo *pResult = nullptr, *ptr = nullptr, hints;
	char *pSendbuf = "Execute_game";
	...
	iResult = getaddrinfo(IPAddress, DEFAULT_PORT, &hints, &pResult);
	if (iResult != 0) {
	...
	for (ptr = pResult; ptr != NULL; ptr = ptr->ai_next) {
		...
		if (iResult == SOCKET_ERROR) {
			if (WSAGetLastError() == WSAECONNREFUSED) {
				printf("Connection refused by server. Trying to connect to the running game.");
	...
	iResult = send(connectSocket, pSendbuf, (int)strlen(pSendbuf), 0);
	if (iResult == SOCKET_ERROR) {
	...
	printf("Request to game server sent.\n");
	...
	do {
		...
		if (iResult > 0)
			printf("Response from game server received.\n");
		else if (iResult == 0)
...
int main(int argc, char *argv[]) {
	...
	pthread_t watchdog;
	char cSavefileKeyts[128];
	char pCurrentDir[MAX_PATH];
	char cCommunicationProtocol[64];
	char cServerAddress[1024];
	char cServerPort[64];
	...
	memset(cServerAddress, 0, sizeof(cServerAddress));
	memset(cServerPort, 0, sizeof(cServerPort));
	...
	pProcessExecutionPath += "\\config\\client.conf";
	//
#ifdef ANDROID
	if(ga_init("/sdcard/ga/android.conf", NULL) < 0) {
		rtsperror("Cannot load configuration file '%s'\n", pProcessExecutionPath.c_str());
		return -1;
	}
#else
	if (argc < 2) {
		if (ga_init(pProcessExecutionPath.c_str()) < 0) {
			rtsperror("Cannot load configuration file '%s'\n", pProcessExecutionPath.c_str());
			return -1;
		}
		printf("Cloud Gaming For Windows config path: %s\n", pProcessExecutionPath.c_str());
	}
	else {
		if (ga_init(argv[1]) < 0) {
			rtsperror("Cannot load configuration file '%s'\n", argv[1]);
	...
	if (ga_conf_readv("communication-protocol", cCommunicationProtocol, sizeof(cCommunicationProtocol))) {
		rtsperror("Communication protocol: %s\n", cCommunicationProtocol);
	}
	if (ga_conf_readv("server-address", cServerAddress, sizeof(cServerAddress))) {
		ga_conf_writev("server-name", cServerAddress);
		rtsperror("Server address: %s\n", cServerAddress);
	}
	...
	if (cCommunicationProtocol && cServerAddress && cServerPort) {
		...
		cServerConnectionPath += "://";
		cServerConnectionPath += cServerAddress;
		cServerConnectionPath += ":";
		cServerConnectionPath += cServerPort;
		cServerConnectionPath += "/desktop";
		ga_conf_writev("server-url", cServerConnectionPath.c_str());

		if (InitiatingConnectionToServer(cServerAddress) < 0) {
			rtsperror("Connection initialization failed.\n");
			return -1;
		}
	}else {
		if (!cCommunicationProtocol) {
			rtsperror("Communication-protocol field empty.\n");
		}
		if (!cServerAddress) {
			rtsperror("Server-address field empty.\n");
		}
		if (!cServerPort) {
			rtsperror("Server-port field empty.\n");
		}
		return -1;
	}

	//
	if(ga_conf_readv("save-key-timestamp", cSavefileKeyts, sizeof(cSavefileKeyts))) {
		savefpKeyts = ga_save_init_txt(cSavefileKeyts);
		rtsperror("*** SAVEFILE: key timestamp saved fo '%s'\n", savefpKeyts ? cSavefileKeyts : "NULL");
	}
	//
	...
	rtspThreadParam.url = strdup(cServerConnectionPath.c_str());
