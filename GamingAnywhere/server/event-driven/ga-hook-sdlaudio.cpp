...
sdlaudio_hook_symbols() {
	...
	if((handle = dlopen(soname, RTLD_LAZY)) != NULL) {
	//////////////////////////////////////////////////
	hook_lib_generic(soname, handle, "SDL_OpenAudio", (void*) hook_SDL_OpenAudio);
	hook_lib_generic(soname, handle, "SDL_PauseAudio", (void*) hook_SDL_PauseAudio);
	hook_lib_generic(soname, handle, "SDL_CloseAudio", (void*) hook_SDL_CloseAudio);
	//
...
hook_SDL_OpenAudio(SDL12_AudioSpec *desired, SDL12_AudioSpec *obtained) {
	...
	if(ret >= 0) {
		...
		ga_error("****************************EXECUTION OF audio_source_setup(bufreq, rtspconf->audio_samplerate, /* XXX: We support only U8 and S16 now */ obtained->format == SDL12_AUDIO_S16 ? 16 : 8, rtspconf->audio_channels ****************************\n");
