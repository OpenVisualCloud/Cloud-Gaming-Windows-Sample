...
hook_SDL2_OpenAudio(SDL_AudioSpec *desired, SDL_AudioSpec *obtained) {
	...
	if(ret >= 0) {
		...
		ga_error("****************************EXECUTION OF audio_source_setup(bufreq, rtspconf->audio_samplerate, /* XXX: We support only U8 and S16 now */ obtained->format == AUDIO_S16 ? 16 : 8, rtspconf->audio_channels ****************************\n");
