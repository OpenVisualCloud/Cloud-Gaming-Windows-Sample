...
SharedParams *g_SharedParams = NULL;
...
vsource_init(int width, int height) {
	...
	do {
		...
		if(video_source_setup_ex(config, SOURCES) < 0) {
			ga_error("Configuration of video source failed. \n");
			return -1;
		}
	} while(0);
#else
	...
	for(i = 0; i < SOURCES; i++) {
		...
		{
			ga_error("Find pipeline source success.\r\n");
		}
	}
	//
...
load_modules() {
	...
	//
	snprintf(module_path, sizeof(module_path),
		BACKSLASHDIR("%s/mod/encoder-mfx", "%smod\\encoder-mfx"),
		ga_root);
