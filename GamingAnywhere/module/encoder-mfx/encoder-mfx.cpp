...
static bool _is_key_frame_required = false;
...
mfxExtAVCEncodedFrameInfo *pextAVCFrameInfo[VIDEO_SOURCE_CHANNEL_MAX]; 
...
mfx_init(void *arg) {
	int iid;
	int RGBmode;
	ga_encoder_cfg_t encCfg;
	char *pipefmt = (char*) arg;

	memset(&encCfg, 0, sizeof(ga_encoder_cfg_t));
	struct RTSPConf *rtspconf = rtspconf_global();
	//
    ga_error("Intel Media Encoder:  mfx_init called\n");
	if(rtspconf == NULL) {
	...
	for(iid = 0; iid < video_source_channels(); iid++) {
		...
		ga_error("Intel MSDK encoder: video source #%d from '%s' (%dx%d).\n",
		...
		encCfg.useRGB = RGBmode;
		encCfg.width  = outputW;
		encCfg.height = outputH;
		encCfg.fps    = rtspconf->video_fps;
		encCfg.bitrateKbps = bitrate;
		encCfg.gopSize = ga_conf_mapreadint("video-specific", "g");
		if(encCfg.gopSize <=0)
			encCfg.gopSize = encCfg.fps;

		encCfg.numOfslices = ga_conf_mapreadint("video-specific", "slices");
		if(encCfg.numOfslices <=0)
			encCfg.numOfslices = 4;

		encCfg.intraRefresh = ga_conf_mapreadint("video-specific", "intra-refresh");

		encCfg.enableQSVFF    =  ga_conf_mapreadint("video-specific", "enableQSVFF");
		encCfg.numOfRoi       =  ga_conf_mapreadint("video-specific", "roi");
        encCfg.listRoiQPI[0]  =  ga_conf_mapreadint("video-specific", "roi_region_0");
        encCfg.listRoiQPI[1]  =  ga_conf_mapreadint("video-specific", "roi_region_1");
        encCfg.listRoiQPI[2]  =  ga_conf_mapreadint("video-specific", "roi_region_2");
        encCfg.listRoiQPI[3]  =  ga_conf_mapreadint("video-specific", "roi_region_3");

		if(mfx_init_internal_adv(iid, encCfg) < 0)
			goto init_failed;
	}
	mfx_initialized = 1;
	ga_error("Intel MSDK encoder: initialized (%d channels).\n", iid);
	return 0;
init_failed:
	mfx_deinit(NULL);
	return -1;
...
mfx_threadproc(void *arg) {
	...
	//
	if(pipe == NULL) {
		ga_error("Intel MSDK encoder: invalid pipeline specified (%s).\n", pipename);
		goto video_quit;
	}
	//
	...
	// start encoding
	ga_error("Intel MSDK encoding started: tid=%ld %dx%d (%dx%d) at %dfps, async=%d, buffer=%dKB.\n",
		ga_gettid(),
	...
	while(mfx_started != 0 && encoder_running() > 0
		...
		encoded_frames++;
      /*  if(encoded_frames==100 && reconfigured == false){
	        // Query the encoder what's the current resolution is
			mfxVideoParam params;
			memset(&params, 0, sizeof(params));
			if((sts = MFXVideoENCODE_GetVideoParam(_session[cid], &params)) == MFX_ERR_NONE)
			{
	            int reconfw=480;
				int reconfh=320;
			    mfx_reconfigure_resolution(_session[cid], reconfw, reconfh);
				ga_error("reconfig happen: surface->Pitch (%d) \n",svppout->Data.Pitch);
				svppout->Info.Width		= MFX_ALIGN16(reconfw);
				svppout->Info.Height	= MFX_ALIGN16(reconfh);
				svppout->Info.CropW		= reconfw;
				svppout->Info.CropH		= reconfh;
				//svppout->Data.Pitch     = MFX_ALIGN32(reconfw);
				reconfigured = true;
				ga_error("after reconfig happen: surface->Pitch (%d), w=%d, h=%d \n",svppout->Data.Pitch,svppout->Info.Width,svppout->Info.Height);
			}
        }*/
...
#undef MSDK_MIN
		...
		// do ENCODE
		// Ben: add average frame QP reporting, which is able to determin the image quality
		// Then decides what deltaQP should be applied?

#if 0
		// add a new 
		if(pextAVCFrameInfo[cid] == NULL){
			pextAVCFrameInfo[cid] =(mfxExtAVCEncodedFrameInfo *)malloc(sizeof(mfxExtAVCEncodedFrameInfo));
		}
	    pextAVCFrameInfo[cid]->Header.BufferId = MFX_EXTBUFF_ENCODED_FRAME_INFO;
	    pextAVCFrameInfo[cid]->Header.BufferSz = sizeof(mfxExtAVCEncodedFrameInfo);
		_mfxbs[cid].ExtParam =(mfxExtBuffer **)(&pextAVCFrameInfo[cid]);
		_mfxbs[cid].NumExtParam = 1;
#endif

    mfxEncodeCtrl ctrl;
    memset((void *)&ctrl, 0, sizeof(ctrl));
    ctrl.FrameType = _is_key_frame_required ? MFX_FRAMETYPE_I
                                            : MFX_FRAMETYPE_UNKNOWN;
    _is_key_frame_required = false;
    sts = mfx_encode_encode(_session[cid], &ctrl, svppout, &_mfxbs[cid], &encsync);
#if 0
		ga_error("video encoder: frame QP=%d \n", pextAVCFrameInfo[cid]->QP);
		ga_error("video encoder: frame MAD=%d \n", pextAVCFrameInfo[cid]->MAD);


		ga_ioctl_reconfigure_t reconf;
		memset(&reconf, 0, sizeof(reconf));
		reconf.id      = 0;
		reconf.deltaQP = (33 - pextAVCFrameInfo[cid]->QP);
		mfx_ioctl(GA_IOCTL_RECONFIGURE_DELTAQP, sizeof(reconf), &reconf);
#endif
		//
		if(sts == MFX_ERR_MORE_DATA)	continue;
		if(sts != MFX_ERR_NONE) {
	...
	while(sts >= MFX_ERR_NONE) {
		sts = mfx_encode_encode(_session[cid], nullptr, NULL, &_mfxbs[cid], &encsync);
	}
	//
video_quit:
	if(pipe) {
...
mfx_ioctl(int command, int argsize, void *arg) {
	...
	case GA_IOCTL_RECONFIGURE:
		...
		if(reconf->bitrateKbps <= 0)
			break;
		sts = mfx_reconfigure(_session[reconf->id], reconf->bitrateKbps);
		if( sts == MFX_ERR_NONE) {
			ga_error("video encoderxxx: reconfigure to bitrate to %dKbps OK\n", reconf->bitrateKbps);
		} else {
			ga_error("video encoderxxx: reconfigure to bitrate to %dKbps failed error code=%d\n", reconf->bitrateKbps, sts);
		}
		break;
	case GA_IOCTL_RECONFIGURE_DELTAQP:
		if(argsize != sizeof(ga_ioctl_reconfigure_t))
			return GA_IOCTL_ERR_INVALID_ARGUMENT;
		//if(reconf->deltaQP <= 0)
		//	break;
		sts = mfx_reconfigure_deltaQP(_session[reconf->id], reconf->deltaQP);
		if( sts == MFX_ERR_NONE) {
			ga_error("video encoderxxx: reconfigure to a new deltaQP\n",  reconf->deltaQP);
		} else {
			//ga_error("video encoderxxx: reconfigure to bitrate to %dKbps failed error code=%d\n", reconf->bitrateKbps, sts);
		}
		break;
  case GA_IOCTL_REQUEST_KEYFRAME:
    _is_key_frame_required = true;
    break;
	default:
...
module_load() {
	...
	struct RTSPConf *rtspconf = rtspconf_global();
	//
	ga_error("Intel Media Encoder:  loaded\n");
	bzero(&m, sizeof(m));
