...
#endif
	if((mem = (mfx_buffer_t*)_aligned_malloc(sizeof(mfx_buffer_t) + nbytes + ALIGNMENT, 4096)) == NULL) {
		bafa_error("ba_alloc: malloc failed (%d+%d+%d)\n",
			sizeof(mfx_buffer_t), nbytes, ALIGNMENT);
		return MFX_ERR_MEMORY_ALLOC;
	}
	memset(mem, 0, sizeof(mfx_buffer_t) + nbytes + ALIGNMENT);
	mem->type = type;
	mem->nbytes = nbytes;
	mem->alignment = ((unsigned) (((mfxU8*) mem) + sizeof(mfx_buffer_t))) & (ALIGNMENT-1);
	mem->alignment = 64;// ALIGNMENT - mem->alignment;
	mem->alignment &= (ALIGNMENT - 1);
	*mid = (mfxHDL) mem;
	return MFX_ERR_NONE;
}
